...
bg_var = background.points.buffer(variegatus[,2:3], radius = 200000, n = 10*nrow(variegatus), mask = Env[[1]])
bg_tri = background.points.buffer(tridactylus[,2:3], radius = 200000, n = 10*nrow(tridactylus), mask = Env[[1]])

# Get environmental data
extract_var = na.omit(cbind(variegatus[,2:3], extract(Env, variegatus[,2:3]), rep(1, nrow(variegatus))))
extract_tri = na.omit(cbind(tridactylus[,2:3], extract(Env, tridactylus[,2:3]), rep(1, nrow(tridactylus))))

colnames(extract_var)[ncol(extract_var)] = 'occ'
colnames(extract_tri)[ncol(extract_tri)] = 'occ'

extbg_var = na.omit(cbind(bg_var, extract(Env, bg_var), rep(0, nrow(bg_var))))
extbg_tri = na.omit(cbind(bg_tri, extract(Env, bg_tri), rep(0, nrow(bg_tri))))
...
#Error in quantile.default(spr, th.sp) : 
